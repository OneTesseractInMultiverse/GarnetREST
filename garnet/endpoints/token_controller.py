...
def post_token():
    ...
    if user is not None:
        if user.authenticate(password):
            ...
            # ACCESS TOKEN
            access_token_expires = datetime.timedelta(hours=app.config['JWT_ACCESS_TOKEN_VALIDITY_HOURS'])
            access_token = create_access_token(identity=user.user_id, expires_delta=access_token_expires)

            # REFRESH TOKEN
            refresh_token_expires = datetime.timedelta(days=app.config['JWT_REFRESH_TOKEN_VALIDITY_DAYS'])
